...
新機能の活用事例として「外部APIの応答待ち」や「人による承認フロー」が多く紹介されていますが、弊社ではIoT案件に携わることも多いため、「**大量のデータ処理に活用できないか**」という点に着目しました。
...
Lambdaは基本的に **「短時間」** で **「ステートレス」** な処理を行うために設計されており、15分の実行時間制限がありました。

これまで、長時間実行や複数ステップにまたがる処理を実装するためには、Lambdaを複数利用できるStep Functionsで実装されている方も多いのではないでしょうか。

### 2-2. durable functionsの主な特徴

1つの関数内で個別に存在する `step` という処理がそれぞれ **15 分以内** に設定することで、従来のLambdaの15分制約を実質的に回避できます。また`wait`で処理を一旦停止したあと再開できます。  
...
まずは基礎となる`step` と `wait`をまず押さえていきましょう。
...
| 3: step 8分 → step 8分（連続） | 16分  | ❌ タイムアウト発生（中断を挟まずに連続実行し、15分制限に達してしまったため） |
...
（上記のサンプルコードをもとに、実行時間`1秒/2秒/3秒/4秒/5秒`を20セット実行しました）
