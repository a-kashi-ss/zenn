...
| 定義方法 | すべて明示的に記述する必要がある | テンプレートの自動生成によりコード量を最小限に押さえられる |
| コード量 | 構成が複雑になると増大する | 自動生成や抽象化により押さえられる |
...
今回使用したコンストラクト`Vpc`はL2であり、使用することでネットワーク構築に必要な他のリソースが適切な設定値で一緒に作成されます。VPC ・サブネット・ルートテーブルに加えてインターネットゲートウェイやアタッチメントも自動作成されています。

`Vpc`に該当するL1のコンストラクトは`CfnVPC`でVPC ・サブネット・ルートテーブルのみ用意されます。
