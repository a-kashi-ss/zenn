...
| 特徴      | ECS on Fargate             | ECS on EC2                               |
| ------- | -------------------------- | ---------------------------------------- |
| 管理責任    | AWS（サーバーレス）                | ユーザー（セルフマネージド）                           |
| インフラ管理  | 不要                         | 必要（OSパッチ、スケーリングなど）                       |
| カスタマイズ性 | 低い（OSレベルのアクセス不可）           | 高い（OSレベルで制御可能）                           |

> 業務外でチュートリアルを見ながらEC2でデプロイをしたことがあり、両者を比較するとFargateの方が設定項目が少ない印象を受けました。
> コストに関する細かい比較はまだ自身ではできていないのですが、 コストに関する記事は[こちら](https://docs.aws.amazon.com/ja_jp/AmazonECS/latest/developerguide/Welcome.html)がわかりやすかったです。
...
※ Fargateのデプロイ時にエラーが発生したため、まずはFargateの自動停止機能(circuit breaker)を想定し、関連設定の見直しを行っていました。しかし、設定に問題は見られず、最終的にコンテナイメージのビルドプロセスに不備が根本原因でした。
...
補足情報として、ECSと関連性が高いサービスを2つ記載しておきます。

### 5-1. ロードバランサー

ロードバランサーとは、ウェブサイトへのアクセスなど、外部からの通信によるサーバにかかる負荷を、均等に振り分けるためのリソースのことです。
HTTPやHTTPSトラフィックの負荷分散を行うロードバランサーが**ALB**（Application Load Balancer）です。

サービス作成時にALBを紐付けすることで、ECSが下記の処理を自動で行います。
...
WAFはアプリケーションをサイバー攻撃から守るセキュリティサービスの1つです。
アプリケーションを保護するために、攻撃を検知し遮断する対策を取ることができます。

さまざまな機能がありますが、下記に主要な3つをご紹介します。

- IP制限機能
  - 悪意のあるIPアドレスからのアクセスを個別で制限が可能。
- レートベースルール機能
  - 一定時間内における単一のIPアドレスからのアクセス回数を制限が可能。
- Bot対策機能
  - アプリケーションのリソースを過剰に消費したり、不正に情報収集をする悪意のあるBotへの対策が可能。
